...
游戏内外共享一个聊天室——赛前你就能和 AI 闲聊，@谁谁就会回应你；游戏结束后，AI 会主动复盘这局表现，吐槽队友、庆祝胜利、互相 @ 对方，像一群真正的朋友打完一局之后的样子。
...
AI 不是一成不变的。每局游戏结束后，每位 AI 会回顾自己的表现，沉淀经验教训，更新个人策略。这些经验按角色和板子独立积累——三月七玩狼人时学到的伪装技巧，不会干扰她玩预言家时的查验策略。

经验存储在每个角色的 `experience.json` 中，结构为 `{板子: {角色: "经验内容"}}`。下一局游戏时，AI 会读取自己积累的经验，替代通用的策略指南。打的越多，AI 越强。

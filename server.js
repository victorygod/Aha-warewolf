...
const server = http.createServer((req, res) => {
  ...
  // API 接口：返回配置（debug模式 + 板子列表）
  if (urlPath === '/api/presets') {
    res.writeHead(200, { 'Content-Type': 'application/json' });
    res.end(JSON.stringify({ debugMode: DEBUG_MODE, presets: BOARD_PRESETS }));
    return;
  }
...
function broadcastState() {
  if (!game) {
    // 游戏未开始，发送等待状态
    const boardRoles = getBoardRoles();
    clients.forEach((info, ws) => {
      send(ws, 'state', { phase: 'waiting', players: [], presetId: currentPresetId, presetLocked: currentPresetId !== null, boardRoles });
    });
    return;
  }
  clients.forEach((info, ws) => {
    send(ws, 'state', game.getState(info.playerId));
  });
}
...
function setupGameListeners() {
  ...
  game.on('player:action', ({ playerId, data }) => {
    ...
    sendToPlayer(playerId, 'state', game.getState(playerId));
  });
...
function handleJoin(ws, msg) {
  ...
  broadcastState();
  send(ws, 'state', { ...game.getState(playerId), gameStarted: false });
}

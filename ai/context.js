    ...
    switch (msg.type) {
      ...
      case 'system':
        ...
        if (msg.content.includes('情侣') || msg.content.includes('是情侣')) {
          ...
          if (match) {
            lines.push(`[私密][情侣]你的伴侣:${match[1]}号`);
          } else {
            lines.push(`[私密][情侣]${msg.content}`);
          }
...
function formatAction(msg, players) {
  ...
  if (meta?.targetId !== undefined && meta?.result) {
    ...
    return `[私密][预言家]${actorPos}号${actorName}:${pos}号${name}=${result}`;
  }
  ...
  if (content.includes('守护')) {
    ...
    if (match) {
      return `[私密][守卫]${actorPos}号${actorName}:守护${match[1]}号${match[2]}`;
    }
  }
  ...
  if (content.includes('解药') || content.includes('救了')) {
    ...
    if (match) {
      return `[私密][女巫]${actorPos}号${actorName}:救${match[1]}号${match[2]}`;
    }
  }
  ...
  if (content.includes('毒杀') || content.includes('毒了')) {
    ...
    if (match) {
      return `[私密][女巫]${actorPos}号${actorName}:毒${match[1]}号${match[2]}`;
    }
  }
...
function formatWolfVoteResult(msg, players) {
  ...
  if (msg.content) {
    ...
    if (match) {
      lines.push(`[私密]最终击杀：${match[1]}号${match[2]}`);
    }
  }

...
class RandomAgent {
  ...
  async decide(context) {
    ...
    // 确保 alivePlayers 有值
    if (!alivePlayers) {
      getLogger().error(`[RandomAgent] alivePlayers 不存在：playerId=${this.playerId}, phase=${phase}`);
      return { type: 'skip' };
    }
  ...
  passBadgeAction(alivePlayers) {
    ...
    const target = others[Math.floor(Math.random() * others.length)];
    return { type: 'passBadge', target: String(target.id) };
  }
  ...
  logContext(context) {
    ...
    if (!player) {
      getLogger().error(`[RandomAgent] player 不存在：playerId=${this.playerId}, game.players=${this.game?.players?.length || 0}`);
      return;
    }
